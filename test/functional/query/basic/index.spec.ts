...
describe('query > basic', async () => {
  ...
  it('should be able to select single column', async () => {
    let ctx = new DbSet(Model);
  ...
  it('should be able to select specific columns', async () => {
    let ctx = new DbSet(Model);
    let loadModelQuery = ctx.select(x => x.id).toList.query;
    expect(loadModelQuery).to.be.equalIgnoreCase('SELECT id FROM model');

    let results = await ctx.select(x => ({ nameAlias: x.name, idAlias: x.id })).first();
    expect(results).to.be.eql({ nameAlias: 'name', idAlias: 'id' });
